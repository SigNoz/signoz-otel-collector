		...
		UpItems: []Operation{
			CreateTableOperation{
				...
				Columns: []Column{
					...
					{Name: "attribute_string_db$$system", Type: LowCardinalityColumnType{ColumnTypeString}, Default: "if(mapContains(attributes_string, 'db.system.name'), attributes_string['db.system.name'], attributes_string['db.system'])", Codec: "ZSTD(1)"},
		...
		UpItems: []Operation{
			...
			AlterTableAddColumn{
				...
				Column: Column{
					...
					Type:    ColumnTypeBool,
					Default: "mapContains(attributes_string, 'db.system.name') OR mapContains(attributes_string, 'db.system')",
					Codec:   "ZSTD(1)",
				},
			},
			AlterTableAddColumn{
		...
		UpItems: []Operation{
			ModifyQueryMaterializedViewOperation{
				...
				Query: `SELECT
							...
							count(*) AS total_count,
							toStartOfMinute(timestamp) AS timestamp,
							COALESCE(NULLIF(resources_string['deployment.environment.name'], ''), NULLIF(resources_string['deployment.environment'], '')) AS deployment_environment,
							resources_string['k8s.cluster.name'] AS k8s_cluster_name,
							resources_string['k8s.namespace.name'] AS k8s_namespace_name
						FROM signoz_traces.signoz_index_v3
						WHERE (dest != '') AND (kind != 2)
			...
			ModifyQueryMaterializedViewOperation{
				...
				Query: `SELECT
							...
							count(*) AS total_count,
							toStartOfMinute(B.timestamp) AS timestamp,
							COALESCE(NULLIF(B.resources_string['deployment.environment.name'], ''), NULLIF(B.resources_string['deployment.environment'], '')) AS deployment_environment,
							B.resources_string['k8s.cluster.name'] AS k8s_cluster_name,
							B.resources_string['k8s.namespace.name'] AS k8s_namespace_name
						FROM signoz_traces.signoz_index_v3 AS A, signoz_traces.signoz_index_v3 AS B
						WHERE (A.resource_string_service$$name != B.resource_string_service$$name) AND (A.span_id = B.parent_span_id)
