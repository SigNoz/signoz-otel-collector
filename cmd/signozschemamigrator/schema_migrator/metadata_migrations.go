	...
	},
	{
		...
		UpItems: []Operation{
			CreateTableOperation{
				...
				Engine: MergeTree{
					OrderBy:     "(signal, column_name, column_type, field_context, field_name, release_time)",
			...
			InsertIntoTable{
				...
				LightWeight: true,
				Synchronous: true,
				Columns:     []string{"signal", "column_name", "column_type", "field_context", "field_name", "release_time"},
				Values:      "('logs', 'resources_string', 'Map(LowCardinality(String), Float64)', 'resource', '__all__', 0)",
			},
			InsertIntoTable{
				Database:    "signoz_metadata",
				Table:       "distributed_column_evolution_metadata",
				...
				Synchronous: true,
				Columns:     []string{"signal", "column_name", "column_type", "field_context", "field_name", "release_time"},
				Values:      fmt.Sprintf("('logs', 'resource', 'JSON()', 'resource', '__all__', %d)", time.Now().UnixNano()),
			},
		},
		DownItems: []Operation{
			DropTableOperation{
