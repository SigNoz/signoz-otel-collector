...
func SimulateTracesProcessing(
	...
	outputTraces []ptrace.Traces, collectorErrs []string, err error,
) {
	simulator, simulatorInitCleanup, err := NewCollectorSimulator(
		ctx, PipelineSignalTraces, processorFactories, configGenerator,
	)
	...
	result, apiErr := GetProcessedTracesFromSimulator(
...
func GetProcessedTracesFromSimulator(
	simulator *CollectorSimulator,
	minTraceCount int,
	timeout time.Duration,
) (
