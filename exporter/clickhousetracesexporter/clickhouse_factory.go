...
type writerMaker func(logger *zap.Logger, db clickhouse.Conn, traceDatabase string, spansTable string, indexTable string, errorTable string, attributeTable string, encoding Encoding, delay time.Duration, size int) (Writer, error)
...
func ClickHouseNewFactory(migrations string, datasource string, dockerMultiNodeCluster bool) *Factory {
	...
	return &Factory{
		...
		// },
		makeWriter: func(logger *zap.Logger, db clickhouse.Conn, traceDatabase string, spansTable string, indexTable string, errorTable string, attributeTable string, encoding Encoding, delay time.Duration, size int) (Writer, error) {
			return NewSpanWriter(logger, db, traceDatabase, spansTable, indexTable, errorTable, attributeTable, encoding, delay, size), nil
		},
	}
}
...
func (f *Factory) CreateSpanWriter() (Writer, error) {
	...
	return f.makeWriter(f.logger, f.db, cfg.TraceDatabase, cfg.SpansTable, cfg.IndexTable, cfg.ErrorTable, cfg.AttributeTable, cfg.Encoding, cfg.WriteBatchDelay, cfg.WriteBatchSize)
}
...
func (f *Factory) CreateArchiveSpanWriter() (Writer, error) {
	...
	return f.makeWriter(f.logger, f.archive, "", cfg.TraceDatabase, cfg.SpansTable, cfg.ErrorTable, cfg.AttributeTable, cfg.Encoding, cfg.WriteBatchDelay, cfg.WriteBatchSize)
}
