...
func TestBatch_addMetadata(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			lastSeen:        baseTimestamp + 1000,
			validate: func(t *testing.T, b *batch) {
				// The harness made the first call: 2 entries.
				require.Len(t, b.metadata, 2)

				// Second call with identical identities but a wider window:
				// collapses onto the existing rows and only widens first/last.
				attrs := pcommon.NewMap()
				attrs.PutStr("service.name", "test-service")
				attrs.PutStr("env", "prod")
				...
				// Third call with a DIFFERENT service.name value: a distinct
				// attribute value must produce a new row. Regression guard for
				// signoz#9787, where the old (value-blind) dedup dropped these.
