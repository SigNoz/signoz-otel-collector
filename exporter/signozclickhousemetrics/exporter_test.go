...
func Test_setFirstSeenLastSeen(t *testing.T) {
	const (
		baseTimestamp = int64(1727286182000) // Base unix millisecond timestamp GMT: Wednesday, 25 September 2024 17:43:02
		offset1       = int64(1000)          // 1 second offset
		offset2       = int64(2000)          // 2 second offset
		offset3       = int64(500)           // 0.5 second offset
	)
	...
	testCases := []struct {
		...
		initialFirst        *int64
		initialLast         *int64
		inputUnixMilli      int64
		expectedFirst       int64
		expectedLast        int64
		expectedFirstNotNil bool
		expectedLastNotNil  bool
	}{
		{
			name:                "nil pointers should initialize with input value",
			initialFirst:        nil,
			initialLast:         nil,
			inputUnixMilli:      baseTimestamp,
			expectedFirst:       baseTimestamp,
			expectedLast:        baseTimestamp,
			expectedFirstNotNil: true,
			expectedLastNotNil:  true,
		},
		{
			name:                "nil pointers with different input value",
			initialFirst:        nil,
			initialLast:         nil,
			inputUnixMilli:      baseTimestamp + offset1,
			expectedFirst:       baseTimestamp + offset1,
			expectedLast:        baseTimestamp + offset1,
			expectedFirstNotNil: true,
			expectedLastNotNil:  true,
		},
		{
			name:                "existing pointers with smaller input should update first",
			initialFirst:        intPtr(baseTimestamp + offset1),
			initialLast:         intPtr(baseTimestamp + offset1),
			inputUnixMilli:      baseTimestamp,
			expectedFirst:       baseTimestamp,
			expectedLast:        baseTimestamp + offset1,
			expectedFirstNotNil: true,
		...
		{
			name:                "existing pointers with larger input should update last",
			initialFirst:        intPtr(baseTimestamp),
			initialLast:         intPtr(baseTimestamp),
			inputUnixMilli:      baseTimestamp + offset1,
			expectedFirst:       baseTimestamp,
			expectedLast:        baseTimestamp + offset1,
			expectedFirstNotNil: true,
		...
		{
			name:                "existing pointers with input between first and last",
			initialFirst:        intPtr(baseTimestamp),
			initialLast:         intPtr(baseTimestamp + offset2),
			inputUnixMilli:      baseTimestamp + offset1,
			expectedFirst:       baseTimestamp,
			expectedLast:        baseTimestamp + offset2,
			expectedFirstNotNil: true,
			expectedLastNotNil:  true,
		},
		{
			name:                "existing pointers with input smaller than first",
			initialFirst:        intPtr(baseTimestamp + offset1),
			initialLast:         intPtr(baseTimestamp + offset2),
			inputUnixMilli:      baseTimestamp,
			expectedFirst:       baseTimestamp,
		...
		{
			name:                "existing pointers with input larger than last",
			initialFirst:        intPtr(baseTimestamp),
			initialLast:         intPtr(baseTimestamp + offset1),
			inputUnixMilli:      baseTimestamp + offset2,
			expectedFirst:       baseTimestamp,
			expectedLast:        baseTimestamp + offset2,
			expectedFirstNotNil: true,
			expectedLastNotNil:  true,
		...
		{
			name:                "existing pointers with input equal to first",
			initialFirst:        intPtr(baseTimestamp),
			initialLast:         intPtr(baseTimestamp + offset1),
			inputUnixMilli:      baseTimestamp,
			expectedFirst:       baseTimestamp,
		...
		{
			name:                "existing pointers with input equal to last",
			initialFirst:        intPtr(baseTimestamp),
			initialLast:         intPtr(baseTimestamp + offset1),
			inputUnixMilli:      baseTimestamp + offset1,
			expectedFirst:       baseTimestamp,
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			firstSeen := tc.initialFirst
			lastSeen := tc.initialLast

			// Make copies if pointers are not nil to avoid modifying original test data
			if firstSeen != nil {
				val := *firstSeen
				firstSeen = &val
			}
			...
			resultFirst, resultLast := setFirstSeenLastSeen(firstSeen, lastSeen, tc.inputUnixMilli)

			require.NotNil(t, resultFirst, "resultFirst should not be nil")
			require.NotNil(t, resultLast, "resultLast should not be nil")
			assert.Equal(t, *resultFirst, tc.expectedFirst)
			assert.Equal(t, *resultLast, tc.expectedLast)
			assert.True(t, *resultFirst <= *resultLast)
		})
	}
}

func Test_setFirstSeenLastSeen_MultipleCalls(t *testing.T) {
	const (
		baseTimestamp = int64(1727286182000)
		offset1       = int64(1000)
		offset2       = int64(2000)
		offset3       = int64(500)
	)

	t.Run("tracking min and max across multiple calls", func(t *testing.T) {
		var firstSeen, lastSeen *int64

		// First call - initialize
		firstSeen, lastSeen = setFirstSeenLastSeen(firstSeen, lastSeen, baseTimestamp+offset1)
		assert.Equal(t, *firstSeen, baseTimestamp+offset1)
		assert.Equal(t, *lastSeen, baseTimestamp+offset1)

		// Second call - larger value
		firstSeen, lastSeen = setFirstSeenLastSeen(firstSeen, lastSeen, baseTimestamp+offset2)
		assert.Equal(t, *firstSeen, baseTimestamp+offset1)
		assert.Equal(t, *lastSeen, baseTimestamp+offset2)

		// Third call - smaller value
		firstSeen, lastSeen = setFirstSeenLastSeen(firstSeen, lastSeen, baseTimestamp+offset3)
		assert.Equal(t, *firstSeen, baseTimestamp+offset3)
		assert.Equal(t, *lastSeen, baseTimestamp+offset2)

		// Fourth call - value between min and max
		firstSeen, lastSeen = setFirstSeenLastSeen(firstSeen, lastSeen, baseTimestamp+offset1)
		assert.Equal(t, *firstSeen, baseTimestamp+offset3)
		assert.Equal(t, *lastSeen, baseTimestamp+offset2)
	})
