package signozclickhousemetricsv2
...
import (
	...
	internalmetadata "github.com/SigNoz/signoz-otel-collector/exporter/signozclickhousemetricsv2/internal/metadata"
...
var (
	...
	samplesSQLTmpl    = "INSERT INTO %s.%s (env, temporality, metric_name, fingerprint, reduced_fingerprint, unix_milli, value, flags, inserted_at_unix_milli) VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
	timeSeriesSQLTmpl = "INSERT INTO %s.%s (env, temporality, metric_name, description, unit, type, is_monotonic, fingerprint, unix_milli, labels) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
...
func (c *clickhouseMetricsExporter) processGauge(
	batch *batch, metric pmetric.Metric, env string,
	resourceFingerprint, scopeFingerprint *pkgfingerprint.Fingerprint,
	_, scopeFingerprintWithoutAttrs *pkgfingerprint.Fingerprint,
) {
	name := metric.Name()
	...
	for i := 0; i < metric.Gauge().DataPoints().Len(); i++ {
		...
		reducedFingerprint := pkgfingerprint.NewFingerprint(pkgfingerprint.PointFingerprintType, scopeFingerprintWithoutAttrs.Hash(), dp.Attributes(), map[string]string{
			"__temporality__": temporality.String(),
		})
		...
		batch.addSample(&sample{
			...
			reducedFingerprint: reducedFingerprint.HashWithName(name),
			unixMilli:          unixMilli,
			value:              value,
...
func (c *clickhouseMetricsExporter) processSum(
	batch *batch, metric pmetric.Metric, env string,
	resourceFingerprint, scopeFingerprint *pkgfingerprint.Fingerprint,
	_, scopeFingerprintWithoutAttrs *pkgfingerprint.Fingerprint,
) {
...
func (c *clickhouseMetricsExporter) processHistogram(
	b *batch, metric pmetric.Metric, env string,
	resourceFingerprint, scopeFingerprint *pkgfingerprint.Fingerprint,
	_, scopeFingerprintWithoutAttrs *pkgfingerprint.Fingerprint,
) {
	name := metric.Name()
	...
	addSample := func(batch *batch, dp pmetric.HistogramDataPoint, suffix string) {
		...
		batch.addSample(&sample{
			...
			reducedFingerprint: reducedFingerprint.HashWithName(name + suffix),
			unixMilli:          unixMilli,
			value:              value,
			flags:              uint32(dp.Flags()),
		})
	...
	addBucketSample := func(batch *batch, dp pmetric.HistogramDataPoint, suffix string) {
		...
		for i := 0; i < dp.ExplicitBounds().Len() && i < dp.BucketCounts().Len(); i++ {
			...
			reducedFingerprint := pkgfingerprint.NewFingerprint(pkgfingerprint.PointFingerprintType, scopeFingerprintWithoutAttrs.Hash(), pointAttrs, map[string]string{
				"le":              boundStr,
				"__temporality__": temporality.String(),
			})
			fingerprintMap := fingerprint.AttributesAsMap()
...
func (c *clickhouseMetricsExporter) processSummary(
	b *batch, metric pmetric.Metric, env string,
	resourceFingerprint, scopeFingerprint *pkgfingerprint.Fingerprint,
	_, scopeFingerprintWithoutAttrs *pkgfingerprint.Fingerprint,
	...
	addQuantileSample := func(batch *batch, dp pmetric.SummaryDataPoint, suffix string) {
		...
		for i := 0; i < dp.QuantileValues().Len(); i++ {
			...
			reducedFingerprint := pkgfingerprint.NewFingerprint(pkgfingerprint.PointFingerprintType, scopeFingerprint.Hash(), dp.Attributes(), map[string]string{
				"quantile":        quantileStr,
				"__temporality__": temporality.String(),
			})
			fingerprintMap := fingerprint.AttributesAsMap()
...
func (c *clickhouseMetricsExporter) processExponentialHistogram(
	b *batch, metric pmetric.Metric, env string,
	resourceFingerprint, scopeFingerprint *pkgfingerprint.Fingerprint,
	_, scopeFingerprintWithoutAttrs *pkgfingerprint.Fingerprint,
) {
...
func (c *clickhouseMetricsExporter) prepareBatch(ctx context.Context, md pmetric.Metrics) *batch {
	...
	for i := 0; i < md.ResourceMetrics().Len(); i++ {
		...
		resourceCopy := pcommon.NewMap()

		rm.Resource().Attributes().CopyTo(resourceCopy)

		resourceCopy.RemoveIf(func(key string, _ pcommon.Value) bool {
			if key == "service.instance.id" || strings.HasPrefix(key, "k8s.pod") {
				return true
			}
			return false
		})
		...
		resourceFingerprintWithoutAttrs := pkgfingerprint.NewFingerprint(pkgfingerprint.ResourceFingerprintType, pkgfingerprint.InitialOffset, rm.Resource().Attributes(), map[string]string{})
		...
		for j := 0; j < rm.ScopeMetrics().Len(); j++ {
			...
			scopeFingerprintWithoutAttrs := pkgfingerprint.NewFingerprint(pkgfingerprint.ScopeFingerprintType, resourceFingerprintWithoutAttrs.Hash(), sm.Scope().Attributes(), map[string]string{
				"__scope.name__":       sm.Scope().Name(),
				"__scope.version__":    sm.Scope().Version(),
				"__scope.schema_url__": sm.SchemaUrl(),
			})
			...
			for k := 0; k < sm.Metrics().Len(); k++ {
				...
				case pmetric.MetricTypeGauge:
					c.processGauge(batch, metric, env, resourceFingerprint, scopeFingerprint, resourceFingerprintWithoutAttrs, scopeFingerprintWithoutAttrs)
				case pmetric.MetricTypeSum:
					c.processSum(batch, metric, env, resourceFingerprint, scopeFingerprint, resourceFingerprintWithoutAttrs, scopeFingerprintWithoutAttrs)
				case pmetric.MetricTypeHistogram:
					c.processHistogram(batch, metric, env, resourceFingerprint, scopeFingerprint, resourceFingerprintWithoutAttrs, scopeFingerprintWithoutAttrs)
				case pmetric.MetricTypeSummary:
					c.processSummary(batch, metric, env, resourceFingerprint, scopeFingerprint, resourceFingerprintWithoutAttrs, scopeFingerprintWithoutAttrs)
				case pmetric.MetricTypeExponentialHistogram:
					c.processExponentialHistogram(batch, metric, env, resourceFingerprint, scopeFingerprint, resourceFingerprintWithoutAttrs, scopeFingerprintWithoutAttrs)
