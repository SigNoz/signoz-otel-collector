...
func newProcessorImp(mexp *mocks.MetricsExporter, tcon *mocks.TracesConsumer, defaultNullValue *string, temporality string, tb testing.TB) *processorImp {
	...
	callMetricKeyToDimensionsCache, err := cache.NewCache(DimensionsCacheSize)
	if err != nil {
		panic(err)
	}
	dbMetricKeyToDimensionsCache, err := cache.NewCache(DimensionsCacheSize)
	if err != nil {
		panic(err)
	}
	externalCallMetricKeyToDimensionsCache, err := cache.NewCache(DimensionsCacheSize)
	if err != nil {
		panic(err)
	}
...
func verifyConsumeMetricsInput(t testing.TB, input pmetric.Metrics, expectedTemporality pmetric.MetricAggregationTemporality, numCumulativeConsumptions int) bool {
	...
	for ; mi < 3; mi++ {
		assert.Equal(t, "signoz_calls_total", m.At(mi).Name())
	...
	for ; mi < m.Len(); mi++ {
		assert.Equal(t, "signoz_latency", m.At(mi).Name())
...
func TestBuildKeyWithDimensions(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		},
		{
			...
			wantKey: "ab\u0000c\u0000SPAN_KIND_UNSPECIFIED\u0000STATUS_CODE_UNSET\u0000resource_99",
		},
		{
